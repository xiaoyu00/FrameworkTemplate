...
object BankUtils {
    ...
    fun checkBankCard(cardNo: String): Boolean {
        ...
        return if (bit == 'N') {
            false
        } else cardNo[cardNo.length - 1] == bit
    }

    /**
     * 校验银行卡卡号
     *
     * @param nonCheckCodeCardId 银行卡卡号
     * @return char字符
     */
    private fun getBankCardCheckCode(nonCheckCodeCardId: String?): Char {
        if (nonCheckCodeCardId.isNullOrBlank() || !nonCheckCodeCardId.matches("\\d+".toRegex())) {
            return 'N'
        }
     ...
     * 获取银行名称
     *
     * @param code 银行简称[如：建行:CCB]
     * @return 银行名称
     */
    fun getNameOfBank(code: String): String {
        for ((a, i) in (0 until bankBin.size - 165).withIndex()) {
            if (code == bankBin[i + a]) {
